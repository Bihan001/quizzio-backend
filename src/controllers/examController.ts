...
import { scheduleExam, destroyScheduler, parseExam, shuffleExam, removeCorrectOptions, evaluateExam } from '../utils/examFunctions';
...
export const createTables = catchAsync(async (req: Request, res: Response) => {
  ...
  result = await db.execute(query);
  if (!result) throw new CustomError('Exam-Participants table not created!', 500);
  //================================
  ...
  result = await db.execute(query);
  if (!result) throw new CustomError('Private-Exam Emails table not created !', 500);
  //=============================
...
export const getExams = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(200).json(SuccessResponse(rows, 'These are upcoming and ongoing Exams !'));
});
...
export const createExam = catchAsync(async (req: CustomRequest, res: Response) => {
  const db = getDb();
  ...
  if (data.isPrivate && data.allowedUsers.length > 0) {
    ...
    if (!result2) throw new CustomError('Failed to set Private User in Private-Exam-Email table', 500);
  }
  // scheduleExam(data.id, data.date, data.duration);
  return res.status(200).json(SuccessResponse({}, 'Exam Created!'));
});

export const editExam = catchAsync(async (req: CustomRequest, res: Response) => {
  const db = getDb();
  ...
  if (!result2) throw new CustomError('Delete from Private-Exam-Emails failed !', 500);
  ...
  if (isPrivate && allowedUsers.length > 0) {
    ...
    if (!result3) throw new CustomError('Failed to update Private User in Private-Exam-Email table', 500);
  }
...
export const registerInExam = catchAsync(async (req: CustomRequest, res: Response) => {
  const db = getDb();
  let query;
  const { examId, email } = req.body;
  const userId = req.user?.id;
  //check if the ids are valid then insert in exam-participants table
  query = 'select isPrivate,userId as creatorId,count(*) as examExists,finished from `Exam` where `id`=? limit 1';
  let [rows] = await db.execute(query, [examId]);
  // console.log(rows);
  let { isPrivate, creatorId, examExists, finished } = rows[0];
  if (!examExists) throw new CustomError('Exam not Found', 500);
  if (creatorId === userId) throw new CustomError('Creator cannot give Exam !', 500);
  if (isPrivate) {
    query = 'select count(*) as userAllowed from `Private-Exam-Emails` where `email`=? limit 1';
    let [rows] = await db.execute(query, [email]);
    //console.log(rows);
    if (!rows[0].userAllowed) throw new CustomError('Not allowed to enter !', 404);
  }

  query = 'select count(*) as alreadyRegistered from `Exam-Participants` where `participantId`=? and `examId`=? limit 1';
  [rows] = await db.execute(query, [userId, examId]);
  if (rows[0].alreadyRegistered) throw new CustomError('User already Registered !', 500);

  query = 'insert into `Exam-Participants` (`examId`,`participantId`,`virtual`) values(?,?,?)';
  let result = await db.execute(query, [examId, userId, finished]);
  if (!result) throw new CustomError('Registering failed !', 500);
  res.status(200).send('Successfully Registered !');
});

export const startExam = catchAsync(async (req: CustomRequest, res: Response) => {
  const db = getDb();
  const examId = req.params.id;
  const userId = req.user?.id;
  ...
  query = 'select count(*) as userRegistered,virtual from `Exam-Participants` where `participantId`=? limit 1';
  let [rows] = await db.execute(query, [userId]);
  if (!rows[0].userRegistered && examRows[0].isPrivate) throw new CustomError('User not yet Registered !', 500);
  if (rows[0].virtual) return res.status(200).json(SuccessResponse(examRows[0], 'Virtual Exam Started !'));
...
export const submitExam = catchAsync(async (req: CustomRequest, res: Response) => {
  const db = getDb();
  let query;
  const { answers, finishTime, examId } = req.body;
  const participantId = req.user?.id;
  const userId = req.user?.id;
  if (!answers || !finishTime || !participantId || !examId) throw new CustomError('Fields are missing !', 500);
  query = 'update `Exam-Participants` set `answers`=? , `finishTime`=? where  `participantId`=?  and `examId`=?';
  let [rows] = await db.execute(query, [JSON.stringify(answers), finishTime, participantId, examId]);
  if (!rows.affectedRows) throw new CustomError('Error occured while submitting! :(', 500);
  res.status(200).json(SuccessResponse(rows[0], 'Exam Submitted Successfully!'));
});

export const forceEvaluateExam = catchAsync(async (req: Request, res: Response) => {
...
export const getQuestionTypes = catchAsync(async (req: Request, res: Response) => {
  const questionTypes: String[] = ['mcq', 'multipleOptions', 'fillInTheBlanks'];
  res.status(200).json(SuccessResponse(questionTypes, 'The question types are :'));
});
