...
export const registerUser = catchAsync(async (req: Request, res: Response) => {
  const db = getDb();
  const { name, email, password, institution, phoneNumber, dob, address, gender } = req.body;
...
export const loginUser = catchAsync(async (req: Request, res: Response) => {
  ...
  res.status(200).json(SuccessResponse({ token: 'Bearer ' + token, user }, 'User Logged in!'));
});
//==============================================
...
export const getExam = catchAsync(async (req: CustomRequest, res: Response) => {
  ...
  if (!examRows[0].ongoing) throw new CustomError('Exam has not started yet !', 500);
